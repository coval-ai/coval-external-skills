...
- **Prompt**: "Given the transcript, did the agent accurately capture the customer's order including all items, modifications, and special instructions? Return YES if all items, quantities, modifications, and special requests were correctly recorded and confirmed. Return NO if any items were missed, incorrect, or special instructions were ignored."
...
If the user provided a critical requirement in Phase 2, create an additional llm-binary metric:
